...
const DANGER_OPTIONS = [
  { value: "LOW", label: "Easy", color: "danger-bg-low" },
  { value: "MEDIUM", label: "Moderate", color: "danger-bg-medium" },
  { value: "HIGH", label: "Intense", color: "danger-bg-high" },
  { value: "EXTREME", label: "Extreme", color: "danger-bg-extreme" },
];
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-card border border-border/50 rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all"
          />
          {filters.search && (
        ...
        <div className="hidden md:flex items-center px-4 py-3 bg-card rounded-xl border border-border/50">
          <span className="text-sm text-muted-foreground">{totalResults} adventures</span>
        </div>
      </div>

      {isExpanded && (
        <div className="p-5 bg-card border border-border/50 rounded-xl space-y-5 animate-unfold">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3, 4, 5].map((price) => (
                  ...
                  className={cn("flex-1 py-2 rounded-lg text-xs font-bold transition-all border btn-3d",
                    price >= filters.priceRange[0] && price <= filters.priceRange[1] ? "bg-primary/15 border-primary/40 text-primary" : "bg-muted/20 border-border/50 text-muted-foreground hover:border-primary/40"
                  )}>{"$".repeat(price)}</button>
                ))}
              </div>
            </div>
                ...
                {DANGER_OPTIONS.map((opt) => (
                  <button key={opt.value} onClick={() => toggleDangerLevel(opt.value)}
                    className={cn("px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all border btn-3d",
                      filters.dangerLevels.includes(opt.value) ? `${opt.color} border-transparent text-black` : "bg-muted/20 border-border/50 text-muted-foreground hover:border-primary/40"
                    )}>{opt.label}</button>
                ))}
              </div>
            </div>
