...
const OKC_CENTER: [number, number] = [-97.5164, 35.4676]; // [lng, lat] for Mapbox
...
const getDangerColor = (level: Mission["dangerLevel"]) => {
  switch (level) {
    case "LOW": return { bg: "#4CAF50", glow: "rgba(76,175,80,0.6)", label: "Easy" };
    case "MEDIUM": return { bg: "#FFC107", glow: "rgba(255,193,7,0.6)", label: "Moderate" };
    case "HIGH": return { bg: "#FF6D00", glow: "rgba(255,109,0,0.6)", label: "Hard" };
    case "EXTREME": return { bg: "#E53935", glow: "rgba(229,57,53,0.6)", label: "Extreme" };
  }
};
...
const createMarkerSVG = (mission: Mission, isInItinerary: boolean, isDimmed: boolean) => {
    ...
    <svg width="${size}" height="${h}" viewBox="0 0 ${size} ${h}" xmlns="http://www.w3.org/2000/svg" style="opacity:${opacity};${isDimmed ? "filter:grayscale(0.6);" : ""}">
      <defs>
        <radialGradient id="g" cx="40%" cy="35%" r="65%">
          <stop offset="0%" stop-color="${colors.bg}" stop-opacity="0.95"/>
      ...
      <ellipse cx="${size / 2}" cy="${h - 3}" rx="${size * 0.25}" ry="3" fill="${colors.bg}" opacity="0.25"/>
      <path d="M${size / 2} 2 C${size * 0.22} 2 2 ${size * 0.32} 2 ${size * 0.55} c0 ${h * 0.45} ${size / 2 - 2} ${h * 0.6} ${size / 2 - 2} ${h * 0.6} s${size / 2 - 2}-${h * 0.15} ${size / 2 - 2}-${h * 0.6} C${size - 2} ${size * 0.32} ${size * 0.78} 2 ${size / 2} 2z"
            fill="url(#g)" stroke="white" stroke-width="1.5" filter="url(#glow)"/>
      <circle cx="${size / 2}" cy="${size * 0.48}" r="${r}" fill="white" opacity="0.95"/>
      <circle cx="${size / 2}" cy="${size * 0.48}" r="${r * 0.55}" fill="${colors.bg}"/>
      <circle cx="${size / 2}" cy="${size * 0.48}" r="${r * 0.25}" fill="white" opacity="0.7"/>
      ${isInItinerary ? `
        <circle cx="${size - 5}" cy="5" r="7" fill="#E65100" stroke="white" stroke-width="1.5"/>
...
const WarRoomMap = ({
  ...
  const difficultyLevels = [
    { color: "#4CAF50", label: "Easy", count: allMissions.filter((m) => m.dangerLevel === "LOW" && isFiltered(m.id)).length },
    { color: "#FFC107", label: "Moderate", count: allMissions.filter((m) => m.dangerLevel === "MEDIUM" && isFiltered(m.id)).length },
    { color: "#FF6D00", label: "Hard", count: allMissions.filter((m) => m.dangerLevel === "HIGH" && isFiltered(m.id)).length },
    { color: "#E53935", label: "Extreme", count: allMissions.filter((m) => m.dangerLevel === "EXTREME" && isFiltered(m.id)).length },
  ];
  ...
  useEffect(() => {
    ...
    const renderMarkers = () => {
      ...
      new mapboxgl.Marker({ element: hqEl, anchor: "center" })
        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 25, className: "mapbox-popup-dark" }).setHTML(`
            <div style="padding:14px;text-align:center;">
              <p style="font-family:'Orbitron',sans-serif;font-size:11px;font-weight:700;color:hsl(30,15%,12%);letter-spacing:2px;text-transform:uppercase;">OKC Basecamp</p>
              <p style="font-size:10px;color:hsl(30,8%,46%);margin-top:4px;">Mission Control · Ground Zero</p>
              <div style="margin-top:8px;height:1px;background:linear-gradient(90deg,transparent,hsl(24,85%,48%,0.3),transparent)"></div>
              <p style="font-size:9px;color:hsl(30,8%,46%);margin-top:6px;font-family:'Orbitron',sans-serif;">35.47°N · 97.52°W</p>
            </div>
          `)
        )
        .addTo(map.current!);
      markersRef.current.push(new mapboxgl.Marker({ element: hqEl }));
      ...
      allMissions.forEach((mission) => {
        ...
        el.innerHTML = createMarkerSVG(mission, inItinerary, isDimmed);
        ...
        const popupHTML = `
          <div style="width:260px;overflow:hidden;border-radius:12px;">
            <div style="height:110px;background-size:cover;background-position:center;position:relative;background-image:url(${getMissionImage(mission.id)})">
              <div style="position:absolute;inset:0;background:linear-gradient(to top,hsl(36,30%,99%) 0%,hsl(36,30%,99%,0.4) 40%,transparent 100%)"></div>
              <div style="position:absolute;top:8px;right:8px;padding:2px 8px;border-radius:20px;font-size:8px;font-weight:700;font-family:'Orbitron',sans-serif;letter-spacing:1px;text-transform:uppercase;background:${colors.bg};color:white;box-shadow:0 0 12px ${colors.glow}">${mission.dangerLevel}</div>
              <div style="position:absolute;bottom:8px;left:10px;right:10px;">
                <h4 style="font-family:'Playfair Display',serif;font-size:14px;font-weight:700;color:hsl(30,15%,12%);line-height:1.2;text-shadow:0 2px 8px hsl(0,0%,100%,0.5)">${mission.name}</h4>
              </div>
            </div>
            <div style="padding:12px;background:hsl(36,30%,99%);">
              <div style="display:flex;align-items:center;gap:5px;margin-bottom:4px;">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="hsl(24,85%,48%)" stroke-width="2.5"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                <span style="font-size:11px;color:hsl(30,8%,46%)">${mission.city}, ${mission.state}</span>
                <span style="font-size:9px;color:hsl(30,8%,55%);margin-left:auto;font-family:'Orbitron',sans-serif;letter-spacing:0.5px">${mission.distanceFromOKC}h</span>
              </div>
              <div style="font-size:9px;color:hsl(24,85%,48%);margin-bottom:6px;font-weight:600;display:flex;align-items:center;gap:4px;">
              ...
              <div style="height:1px;background:linear-gradient(90deg,transparent,hsl(30,15%,88%),transparent);margin-bottom:8px;"></div>
              <div style="display:flex;align-items:center;justify-content:space-between;">
                <span style="font-weight:700;font-size:14px;font-family:'Orbitron',sans-serif;color:hsl(24,85%,48%);text-shadow:0 0 8px hsl(24,85%,48%,0.3)">${mission.priceEstimate}</span>
              </div>
        ...
        const popup = new mapboxgl.Popup({ offset: [0, -size * 0.6], maxWidth: "280px", className: "mapbox-popup-dark" })
          .setHTML(popupHTML);

        popup.on("open", () => {
          setTimeout(() => {
            const btn = document.querySelector(`button[data-mission-id="${mission.id}"]`);
  ...
  }, [allMissions, itineraryMissions, filteredMissionIds, hasActiveFilter, mapStyle]);
  ...
  useEffect(() => {
    ...
    const fitMap = () => {
      map.current!.fitBounds(bounds, { padding: 80, maxZoom: 7, pitch: 45, bearing: -5, duration: 1200 });
    };

    if (map.current.isStyleLoaded()) {
      fitMap();
    } else {
      map.current.once("style.load", fitMap);
    }
  }, [allMissions]);
  ...
  return (
    <div className={`relative w-full ${isFullscreen ? "fixed inset-0 z-[55]" : ""}`} id="map-section" role="region" aria-label="Adventure map showing mission locations">
      <div className={`relative w-full overflow-hidden rounded-2xl border border-border/20 shadow-[0_8px_40px_hsl(0_0%_0%/0.35),inset_0_1px_0_hsl(0_0%_100%/0.03)] ${isFullscreen ? "h-full rounded-none" : "h-[85vh] min-h-[500px] max-md:h-[500px]"}`}>

        {/* Vignette */}
        <div className="absolute inset-0 z-[2] pointer-events-none" style={{ background: "radial-gradient(ellipse at center, transparent 50%, hsl(36,33%,97%,0.3) 100%)" }} />
        ...
        <div className="absolute top-3 left-3 z-[10]">
          <div className="glass-premium rounded-xl px-4 py-3">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-primary to-primary/50 flex items-center justify-center shadow-[0_0_16px_hsl(var(--primary)/0.3)]" style={{ animation: "pulse-glow 4s ease-in-out infinite" }}>
        ...
        <div className="absolute top-3 right-3 z-[10] flex items-center gap-2">
          <button onClick={cycleStyle}
            className="w-9 h-9 flex items-center justify-center glass-premium rounded-lg text-muted-foreground hover:text-primary transition-all group" title={`Style: ${mapStyle}`} aria-label="Toggle map style">
